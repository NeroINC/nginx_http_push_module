...
static ngx_int_t ngx_http_push_subscriber_get_etag_int(ngx_http_request_t * r) {
	...
	u_char* tag_start 	= if_none_match->data;
	size_t tag_len 	= if_none_match->len;
	...
	tag = ngx_atoi(tag_start, tag_len);

	if( tag == NGX_ERROR ) {
		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "push module: Error parsing If-None-Match header");
		tag = 0;
	}
	else {
		//Debugging only
		//ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "push module: Parse If-None-Match Tag %d", tag);
	}
